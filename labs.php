...
if ($cluster_id > 0) {
    $stmt = $mysqli->prepare('SELECT cluster_id, cluster_name, cluster_description FROM clusters WHERE cluster_id = ? LIMIT 1');
...
$labs_assets = [];
if ($labs) {
    $lab_ids = array_map(function ($lab) {
        return (int) $lab['lab_id'];
    }, $labs);
    $placeholders = implode(',', array_fill(0, count($lab_ids), '?'));
    $types = str_repeat('i', count($lab_ids));
    $stmt = $mysqli->prepare("SELECT lab_id, asset_name FROM assets WHERE lab_id IN ($placeholders) AND asset_status <> 'Disposed'");
    $stmt->bind_param($types, ...$lab_ids);
    $stmt->execute();
    $result = $stmt->get_result();
    while ($row = $result->fetch_assoc()) {
        $lab_id = (int) $row['lab_id'];
        if (!isset($labs_assets[$lab_id])) {
            $labs_assets[$lab_id] = [];
        }
        $labs_assets[$lab_id][] = $row['asset_name'];
    }
    $stmt->close();

    foreach ($labs as &$lab) {
        $lab_id = (int) $lab['lab_id'];
        $lab['assets'] = $labs_assets[$lab_id] ?? [];
    }
    unset($lab);
}
...
$supervisors = [];
foreach ($labs as $lab) {
    $name = trim((string) ($lab['supervisor_name'] ?? ''));
    if ($name !== '') {
...
<body data-login-url="index.php">
        ...
        <div class="main">
            ...
            <section class="content">
                <div class="content-header">
                    <div>
                        <h1><?php echo htmlspecialchars($cluster['cluster_name']); ?></h1>
                        <p>Choose a laboratory to view details and availability.</p>
                    </div>
                    <div class="breadcrumb">Home / <?php echo htmlspecialchars($cluster['cluster_name']); ?></div>
                </div>
                ...
                <div class="card">
                    <div class="banner banner-stack">
                        <div class="banner-head">
                            <div>
                                <h2>Choose a laboratory</h2>
                                <p><?php echo htmlspecialchars($cluster['cluster_description']); ?></p>
                            </div>
                            <div class="banner-links">
                                <a class="btn ghost" href="booking.php">Back to clusters</a>
                            </div>
                        </div>
                        <div class="filters filters-block">
                            <input type="text" id="lab-search" placeholder="Search lab name, supervisor, or asset">
                    ...
                    <div>
                        <p class="muted-text filters-summary" id="lab-search-summary">
                            Total makmal: <?php echo (int) count($labs); ?>
                        </p>
                    </div>
                </div>
    ...
    <script>
        ...
        window.LABS_DATA = <?php echo json_encode(['cluster' => $cluster, 'labs' => $labs]); ?>;
    </script>
