...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $cluster_description = trim($_POST['cluster_description'] ?? '');
    ...
    if (!$errors) {
        ...
        $stmt->bind_param('ss', $cluster_name, $cluster_description);
        if ($stmt->execute()) {
            if (!empty($_SERVER['HTTP_X_REQUESTED_WITH'])) {
                header('Content-Type: application/json');
                echo json_encode([
                    'ok' => true,
                    'cluster_id' => $stmt->insert_id,
        ...
        $stmt->close();
        $errors[] = 'Unable to add cluster. Please try again.';
    }
}
        ...
        <div class="main">
            ...
            <section class="content">
                <div class="content-header">
                    <div>
                        <h1>Add Cluster</h1>
                        <p>Create a new cluster for labs and bookings.</p>
                    </div>
                    <div class="breadcrumb">Home / Booking / Add Cluster</div>
                </div>
                ...
                <div class="card form-card">
                    <form method="POST">
                        <div class="field-group">
                            ...
                            <input type="text" id="cluster_name" name="cluster_name" value="<?php echo htmlspecialchars($cluster_name); ?>" required>
                        </div>
                        <div class="field-group">
                            <label for="cluster_description">Cluster Description</label>
                            <textarea id="cluster_description" name="cluster_description" rows="4"><?php echo htmlspecialchars($cluster_description); ?></textarea>
                        </div>
                        <div class="form-actions">
                            <a class="btn ghost" href="booking.php">Cancel</a>
