...
if ($is_lab_supervisor) {
    ...
    if ($assigned_lab_ids) {
        ...
        if ($supervisor_labs) {
            $asset_stmt = $mysqli->prepare("SELECT lab_id, asset_name FROM assets WHERE lab_id IN ($placeholders) AND asset_status <> 'Disposed' ORDER BY asset_name ASC");
            $asset_stmt->bind_param($types, ...$assigned_lab_ids);
            $asset_stmt->execute();
            $asset_result = $asset_stmt->get_result();
            $assets_by_lab = [];
            while ($row = $asset_result->fetch_assoc()) {
                $lab_id = (int) $row['lab_id'];
                if (!isset($assets_by_lab[$lab_id])) {
                    $assets_by_lab[$lab_id] = [];
                }
                $assets_by_lab[$lab_id][] = $row['asset_name'];
            }
            $asset_stmt->close();

            foreach ($supervisor_labs as &$lab) {
                $lab_id = (int) $lab['lab_id'];
                $lab['assets'] = $assets_by_lab[$lab_id] ?? [];
            }
            unset($lab);
        }
    }
} else {
    $result = $mysqli->query('SELECT cluster_id, cluster_name, cluster_description FROM clusters ORDER BY cluster_name ASC');
    if ($result) {
        while ($row = $result->fetch_assoc()) {
            $clusters[] = $row;
        }
    ...
    foreach ($clusters as $cluster) {
        $cluster_meta[(int) $cluster['cluster_id']] = [
            'labs' => [],
            'assets' => []
        ];
    }
    ...
    if ($cluster_meta) {
        $labs_stmt = $mysqli->prepare('SELECT lab_id, lab_name, cluster_id FROM labs ORDER BY lab_name ASC');
        $labs_stmt->execute();
        $labs_result = $labs_stmt->get_result();
        while ($row = $labs_result->fetch_assoc()) {
            $cluster_id = (int) $row['cluster_id'];
            if (isset($cluster_meta[$cluster_id])) {
                $cluster_meta[$cluster_id]['labs'][] = $row['lab_name'];
            }
        }
        $labs_stmt->close();

        $assets_stmt = $mysqli->prepare("
            SELECT a.asset_name, l.cluster_id
            FROM assets a
        ...
        while ($row = $assets_result->fetch_assoc()) {
            $cluster_id = (int) $row['cluster_id'];
            if (isset($cluster_meta[$cluster_id])) {
                $cluster_meta[$cluster_id]['assets'][] = $row['asset_name'];
            }
        }
        $assets_stmt->close();
    }
...
<head>
    ...
    <title><?php echo $is_lab_supervisor ? 'My Assigned Labs' : 'Cluster Selection'; ?></title>
...
<body data-login-url="index.php">
    <div class="app">
        ...
        <div class="main">
            ...
            <section class="content">
                <div class="content-header content-header-end">
                    <div class="header-actions">
                        <div class="breadcrumb">Home / <?php echo $is_lab_supervisor ? 'My Assigned Labs' : 'Cluster Selection'; ?></div>
                    </div>
                </div>
                    ...
                    <div class="card">
                        <div class="banner banner-stack">
                            <div class="banner-head">
                                <div>
                                    <h2>Assigned laboratory cards</h2>
                                    <p>Your booking view is limited to labs assigned to your supervisor role and cluster scope.</p>
                                </div>
                            </div>
                            <div class="filters filters-block">
                                <input type="text" id="lab-search" placeholder="Search lab name, cluster, supervisor, or asset">
                                <button class="btn primary" type="button" id="lab-search-btn">Search</button>
                            </div>
                        </div>
                        <div>
                            <p class="muted-text filters-summary" id="lab-search-summary">
                                Total makmal: <?php echo (int) count($supervisor_labs); ?>
                            </p>
                        </div>
                    </div>
                ...
                <?php else: ?>
                    <div class="card">
                        <div class="banner">
                            <div>
                                <h2>Filter by Industry, Lab, or Asset</h2>
                                <p>Refine the cluster list using lab or asset keywords.</p>
                            </div>
                            <div class="banner-links">
                                <?php if ($is_super_admin): ?>
                                    <button class="btn primary" id="add-cluster-btn" type="button">Add Cluster</button>
                                <?php endif; ?>
                            </div>
                        </div>
                        <div class="filters">
        ...
        <div class="modal" id="cluster-modal">
            <div class="modal-content">
                <div class="modal-header">
                    <h2>Add Cluster</h2>
                    <button class="icon-button" data-close="cluster-modal">x</button>
                </div>
                <form class="modal-body" id="cluster-form">
                    ...
                    <div class="field-group">
                        ...
                        <textarea id="cluster_description" name="cluster_description" rows="4"></textarea>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn ghost" data-close="cluster-modal">Cancel</button>
                        <button type="submit" class="btn primary">Save Cluster</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <script>
        ...
        window.LABS_DATA = <?php echo json_encode($is_lab_supervisor
            ? ['labs' => $supervisor_labs]
            : ['clusters' => $clusters, 'clusterMeta' => $cluster_meta]); ?>;
    </script>
    <script src="assets/app.js?v=<?php echo (int) filemtime(__DIR__ . '/assets/app.js'); ?>"></script>
    <script src="assets/booking.js?v=<?php echo (int) filemtime(__DIR__ . '/assets/booking.js'); ?>"></script>
</body>
</html>
